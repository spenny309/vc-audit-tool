...
def test_full_pipeline_produces_valid_report_for_all_sectors(sector):
    request = ValuationRequest(company_name="Test Co", sector=sector, revenue_mm=50.0)
...
def test_fair_value_is_multiple_times_revenue():
    request = ValuationRequest(company_name="Test Co", sector=Sector.SAAS, revenue_mm=10.0)
    report = CompsModel().run(request)
