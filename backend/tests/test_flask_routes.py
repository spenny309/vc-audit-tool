...
def test_valuate_returns_report(client):
    response = client.post(
        "/api/valuate",
        json={"company_name": "Basis AI", "sector": "SaaS", "revenue_mm": 10.0},
    )
    assert response.status_code == 200
    data = json.loads(response.data)
    assert data["company_name"] == "Basis AI"
...
def test_valuate_rejects_invalid_sector(client):
    response = client.post(
        "/api/valuate",
        json={"company_name": "X", "sector": "NotASector", "revenue_mm": 10.0},
    )
...
def test_valuate_rejects_negative_revenue(client):
    response = client.post(
        "/api/valuate",
        json={"company_name": "X", "sector": "SaaS", "revenue_mm": -5.0},
    )
    assert response.status_code == 400
